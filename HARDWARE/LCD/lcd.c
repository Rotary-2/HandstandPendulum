...
uint8_t FONT_XUAN[] = {0x00,0x00,0x08,0x01,0x08,0x06,0x8F,0xF8,0x69,0x02,0x09,0x01,0x09,0xFE,0x08,0x01,0x14,0x06,0xE4,0xF8,0x24,0x04,0x27,0xFE,0x24,0x22,0x24,0x22,0x27,0x22,0x20,0x02,};/*"旋",0*/
uint8_t FONT_ZHUAN[] = {0x00,0x00,0x13,0x10,0x1D,0x18,0xF1,0x10,0x17,0xFF,0x11,0x20,0x11,0x20,0x02,0x00,0x12,0x40,0x12,0xD0,0x17,0x48,0xFA,0x44,0x12,0x4B,0x12,0x50,0x12,0x60,0x02,0x00,};/*"转",1*/
uint8_t FONT_BI[] = {0x00,0x00,0x04,0x00,0x08,0x00,0xFF,0x00,0xAA,0x7F,0xAA,0x54,0xAA,0x54,0xEF,0x54,0x00,0x54,0x4A,0x54,0x6A,0x54,0x5A,0x55,0xCF,0x7F,0x5A,0x00,0x6A,0x00,0x48,0x00,};/*"臂",2*/
uint8_t FONT_BAI[] = {0x00,0x00,0x08,0x40,0x08,0x42,0x08,0x81,0xFF,0xFE,0x09,0x00,0x02,0x20,0x78,0x22,0x49,0x27,0x49,0x2A,0x79,0x32,0x4F,0xE2,0x79,0x22,0x49,0x2A,0x49,0x26,0x78,0x23,};/*"摆",3*/
uint8_t FONT_GAN[] = {0x00,0x00,0x08,0x20,0x08,0xC0,0x0B,0x00,0xFF,0xFF,0x09,0x00,0x08,0xC0,0x01,0x00,0x41,0x00,0x41,0x00,0x41,0x00,0x7F,0xFF,0x41,0x00,0x41,0x00,0x41,0x00,0x01,0x00,};/*"杆",4*/
uint8_t FONT_JIAO[] = {0x00,0x00,0x00,0x00,0x04,0x01,0x08,0x06,0x17,0xF8,0x24,0x90,0xE4,0x90,0x24,0x90,0x24,0x90,0x27,0xFE,0x24,0x90,0x2C,0x90,0x34,0x92,0x04,0x91,0x07,0xFE,0x00,0x00,};/*"角",5*/
uint8_t FONT_DU[] = {0x00,0x00,0x00,0x02,0x00,0x0C,0x3F,0xF1,0x24,0x01,0x24,0x21,0x24,0x32,0x3F,0xAA,0xA4,0xA4,0x64,0xA4,0x24,0xA4,0x3F,0xAA,0x24,0x32,0x24,0x01,0x24,0x01,0x20,0x01,};/*"度",6*/
uint8_t FONT_MAOHAO[] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x03,0x60,0x03,0x60,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,};/*"：",7*/
	
/**
 * @brief       使用字模在指定位置显示一个字符
 * @param       x,y  : 坐标
 * @param       chr  : 要显示的字符:
 ...
 * @retval      无
 */
void lcd_show_xfont(uint16_t x, uint16_t y, const char* chr, uint8_t size, uint8_t mode, uint16_t color)
{
    uint8_t temp, t1, t;
    uint16_t y0 = y;
    uint8_t csize = 0;
    ...
    csize = 32; /* 得到字体一个字符对应点阵集所占的字节数 */
		if (strcmp(chr, "旋") == 0) {
        pfont = FONT_XUAN;
    }
		else if (strcmp(chr, "转") == 0) {
        pfont = FONT_ZHUAN;
    }
		else if (strcmp(chr, "臂") == 0) {
        pfont = FONT_BI;
    }
		else if (strcmp(chr, "摆") == 0) {
        pfont = FONT_BAI;
    }
		else if (strcmp(chr, "杆") == 0) {
        pfont = FONT_GAN;
    }
		else if (strcmp(chr, "角") == 0) {
        pfont = FONT_JIAO;
    }
		else if (strcmp(chr, "度") == 0) {
        pfont = FONT_DU;
    }
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)   /* 一个字节8个点 */
        {
            ...
            {
								POINT_COLOR=BLACK; 
                LCD_DrawPoint(x, y);        /* 画点出来,要显示这个点 */
            }
            else if (mode == 0)     /* 无效点,不显示 */
            {
								POINT_COLOR=WHITE; 
                LCD_DrawPoint(x, y); /* 画背景色,相当于这个点不显示(注意背景色由全局变量控制) */
            }
